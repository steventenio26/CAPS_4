...
if ((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
	...
	$name_notification  = 'Submit Assignment file name'." ".'<b>'.$name.'</b>'; 
        ...
        if (!file_exists($newname)) {
            ...
            if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
				...
				$query = mysqli_query($conn,"select * from student_assignment where assignment_id = '$assignment_id' and student_id='$session_id'")or die(mysqli_error());
   ...
   <?php
            ...
            if ($errflag) {
                ...
                session_write_close(); ?>
       <script>
   window.location = 'downloadable1.php<?php echo '?id='.$get_id;  ?>';
   </script>
   <?php
