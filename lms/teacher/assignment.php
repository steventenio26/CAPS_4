  ...
  if(!empty($_FILES['file_upload']) && $_FILES['file_upload']['error'] == 0){
    ...
    if(!file_exists($upload_dir)){
      if(move_uploaded_file($tname,$upload_dir)){
        mysqli_query($conn,"INSERT INTO notification (teacher_class_id,notification,date_of_notification,link) VALUES('$get_id','$name_notification',NOW(),'assignment_student.php')")or die(mysqli_error());

        $sql = "INSERT INTO assignment (fdesc,floc,fdatein,teacher_id,class_id,fname) VALUES ('$description','$upload_dir',NOW(),'$session_id','$get_id','$name')";
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
          ...
          <nav aria-label="breadcrumb">
            ...
            <div class="btn-toolbar mb-2 mr-4">
                <button class="btn btn-sm btn-outline-info" data-toggle="modal" data-target="#exampleModalCenter"><span data-feather="book-open"></span>
                Add Assignment</button>
            </div>
            <?php }
            ?>
          </div>
          <?php include "upload_assignment.php"; ?>
          <?php include "assignment_list.php";?>
